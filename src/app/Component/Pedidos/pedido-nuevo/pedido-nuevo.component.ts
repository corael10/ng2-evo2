...
export class PedidoNuevoComponent implements OnInit {
  ...
  OnEmpleadoSustituto(){
    ...
    {
      this.sustituto = true;
      $('#empleadoSustituto').show();
    }
    else{
      this.sustituto = false;
      $('#empleadoSustituto').hide();
      this.model.empleadosustituto='';
    }
  ...
  public submit() {
    ...
    if (this.model.cliente == '') {
      ...
      //console.log('empleado sustituto', this.model.empleadosustituto);
      ...
      //this.servicio_ped.addPedido(this.model).subscribe(data => {
        console.log('modelo a insertar ',this.model);
        this.servicio_ped.addPedido(this.model).subscribe(data => { 
      ...
      //this.router.navigate(['/app/pedido-nuevo',this.model]);
      /*$('#Divprod').hide();
      //this.value = this.model.cliente;
      console.log('thismodel cliente ',this.model.cliente);
  ...
  YourCancelEvent(){
    console.log('model al cancelar ', this.model);
    this.router.navigate(['/app/pedidos', this.model]);
  }
  onFecha() {
    $('#empleado').show();
  ...
  agregar_temp() {
    if (this.unidad.unidades == 0 || this.unidad.unidades < 0) {
      ...
      if (this.unidad.unidades > this.productos_temp['unidades'])
      {
      this.msg = 'Solo cuentas con '+this.productos_temp['unidades']+' unidades en Inventario';
      this.header = 'Aviso';
      this.ShowPupop(this.rojo, this.header);
      }
      else{
